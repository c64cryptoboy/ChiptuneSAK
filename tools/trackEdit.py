...
def main():
    ...
    parser.add_argument(
        "track", type=str, help="Track either number (starting with 1) or name"
    )
    ...
    quant_group.add_argument(
        "-d", "--quantizelongticks", type=int, help="quantize long notes to ticks"
    )
    parser.add_argument(
        "-m", "--merge", type=int, help="merge track (max num ticks to merge)"
    )
    short_note_group = parser.add_mutually_exclusive_group(required=False)
    short_note_group.add_argument(
        "-r", "--removeshort", type=int, help="remove notes <= tick value"
    )
    short_note_group.add_argument(
        "-l",
        "--setminnotelen",
