...
def main():
    ...
    parser.add_argument(
        "-z",
        "--removekeyswitchnotes",
        action="store_true",
        help="remove keyswitch notes",
    )
    ...
    parser.add_argument(
        ...
        "--moveticks",
        type=str,
        help="move ticks lXXXX for left and rXXXX for right",
    )
    ...
    quant_group.add_argument(
        "-a", "--quantizeauto", action="store_true", help="Auto-quantize"
    )
    quant_group.add_argument(
        "-q", "--quantizenote", type=str, help="quantize to a note value"
    )
    quant_group.add_argument(
        "-c", "--quantizeticks", type=int, help="quantize to ticks"
    )
    parser.add_argument(
        "-r", "--removepolyphony", action="store_true", help="remove polyphony"
    )
    parser.add_argument("-b", "--qpm", type=int, help="set qpm")
    parser.add_argument(
        "-j", "--timesignature", type=str, help="set time signature e.g. 3/4"
    )
    parser.add_argument(
        "-k", "--keysignature", type=str, help="set key signature, e.g. D, F#m"
    )
