...
def apply_xform(note, xform):
    ...
    """
    return ctsChirp.Note(
        0, note.note_num + xform.transpose, int(note.duration * xform.stretch)
    )
    ...
    for it, t in enumerate(song.tracks):
        ...
        for first_position in range(0, n_notes - min_repeat_length):
            for start_position in range(
                first_position + min_repeat_length, n_notes - min_repeat_length + 1
            ):
                ...
                for i in range(1, n_notes - start_position):
                    start_diff = (
                        notes[start_position + i].start_time
                        - notes[start_position + i - 1].start_time
                    )
                    first_diff = int(
                        (
                            notes[first_position + i].start_time
                            - notes[first_position + i - 1].start_time
                        )
                        * xform.stretch
                        + 0.1
                    )
                    ...
                    if (first_diff != start_diff) or (tmpxf != start_note):
                        ...
                        if i >= min_repeat_length:
                            results.append(
                                Repeat(it, first_position, start_position, i, xform)
                            )
                            i = 0
                        break
                i += 1
...
def find_best_compression(
    song,
    repeats,
    pattern_definition_overhead,
    pattern_definition_cost_note,
    pattern_play_cost,
):
    """
    ...
    for r in sorted(repeats):
        ...
        tmp.append(r)
    pattern_starts.append(
        tmp
    )  # We now have a list of lists of patterns starting at each note position.
    for p in pattern_starts:
        ...
        for pattern_length in lengths:
            ...
            cost = (
                pattern_definition_overhead
                + pattern_definition_cost_note * pattern_length
                + nloops * pattern_play_cost
            )
            total = pattern_definition_overhead + pattern_definition_cost_note * (
                pattern_length * nloops
            )
            net = total - cost
            coverage = pattern_length * nloops
            coverage_pct = coverage / len(song.tracks[p[0].track].notes) * 100.0
            if net > 0:
                print(
                    "%d loops of %d cost = %d for %d base (net %d)"
                    % (nloops, pattern_length, cost, total, net)
                )
                print(
                    "Coverage = %d notes (%.1lf%% of track)" % (coverage, coverage_pct)
                )
                print("Track Start   m/b    Repeat   m/b    Length  XForm")
                print("----- ----- -------- ------ -------- ------  -----")
                for rp in poss:
                    smb = str(
                        song.get_measure_beat(
                            song.tracks[current_track].notes[rp.start].start_time
                        )
                    )
                    rmb = str(
                        song.get_measure_beat(
                            song.tracks[current_track].notes[rp.repeat_start].start_time
                        )
                    )
                    print(
                        "%4d %5d %8s %6d %8s %6d  %s"
...
if __name__ == "__main__":
    ...
    print(
        ...
        (in_song.qticks_notes, in_song.qticks_durations),
        "ticks",
    )
