...
def ascii_to_prg(ascii_prg, start_of_basic, max_line_len, basic_tokens):
    ...
    for line in lines:
        ...
        if rem_split_loc != -1:
            remBytes = ascii_to_petscii(
                bytearray(line[rem_split_loc + rem_len :], "latin-1")
            )
        ...
        tokenized_lines += (
            little_endian_bytes(mem_pointer)
            + little_endian_bytes(line_num)
            + tokenized_line
        )
