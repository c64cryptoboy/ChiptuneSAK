...
DURATIONS = {
    "US": {
        ...
        Fraction(3, 32): "dotted sixty-fourth",
        Fraction(1, 12): "thirty-second triplet",
        Fraction(1, 16): "sixty-fourth",
        Fraction(1, 24): "sixty-fourth triplet",
    },
    "UK": {
        Fraction(8, 1): "breve",
        Fraction(6, 1): "dotted semibreve",
        Fraction(4, 1): "semibreve",
        Fraction(3, 1): "dotted minim",
        Fraction(2, 1): "minim",
        Fraction(4, 3): "minim triplet",
        Fraction(3, 2): "dotted crochet",
        Fraction(1, 1): "crochet",
        Fraction(3, 4): "dotted quaver",
        Fraction(2, 3): "crochet triplet",
        Fraction(1, 2): "quaver",
        Fraction(3, 8): "dotted semiquaver",
        Fraction(1, 3): "quaver triplet",
        Fraction(1, 4): "semiquaver",
        Fraction(3, 16): "dotted demisemiquaver",
        Fraction(1, 6): "semiquaver triplet",
        Fraction(1, 8): "demisemiquaver",
        Fraction(3, 32): "dotted hemidemisemiquaver",
        Fraction(1, 12): "demisemiquaver triplet",
        Fraction(1, 16): "hemidemisemiquaver",
        Fraction(1, 24): "hemidemisemiquaver triplet",
    },
}
...
ARCH = {
    ...
    "NTSC-C64": ArchDescription(
        system_clock=1022727,  # The "new" NTSC 6567R8
        cycles_per_line=65,
        lines_per_frame=263,
        visible_lines=235,
    ),
    # Old NTSC C64 and C128 (1Mhz mode)
    "NTSC-R56A": ArchDescription(
        system_clock=1022727,  # The "old" NTSC 6567R56A
        cycles_per_line=64,
        lines_per_frame=262,
        visible_lines=234,
    ),
    # PAL C64 and C128 (1Mhz mode)
    "PAL-C64": ArchDescription(
        system_clock=985248,  # 6569 chip
        cycles_per_line=63,
        lines_per_frame=312,
        visible_lines=284,
    ),
    "NTSC-VIC20": ArchDescription(
        system_clock=1022727,  # 6560-101 chip
        cycles_per_line=65,
        lines_per_frame=261,
        visible_lines=233,
    ),
    "PAL-VIC20": ArchDescription(
        system_clock=1108405,  # 6561-101 chip
        cycles_per_line=71,
        lines_per_frame=312,
        visible_lines=284,
    ),
...
def midi_num_to_freq_arch(midi_num, arch, tuning=CONCERT_A):
    ...
    if arch not in ("NTSC-C64", "PAL-C64"):
        raise ChiptuneSAKValueError(
            "Error: arch type not supported for freq conversion"
        )
    ...
    return round(
        (0x1000000 / ARCH[arch].system_clock) * midi_num_to_freq(midi_num, tuning)
    )
...
def freq_to_midi_num(freq, tuning=CONCERT_A):
    ...
    """
    midi_num_float = (log2(freq) - log2(tuning)) * 12.0 + A4_MIDI_NUM
...
def project_to_absolute_path(file_path):
    ...
    return os.path.normpath(
        os.path.join(Path(__file__).parent.parent.absolute(), file_path)
    )
