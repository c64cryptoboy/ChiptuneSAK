...
class SongTestCase(unittest.TestCase):
    def setUp(self):
        self.test_song = ctsMidi.MIDI().to_chirp(
            TEST_SONG, quantization="16", polyphony=False
        )
    ...
    def test_tracks(self):
        ...
        self.assertTupleEqual(
            tuple(t.name for t in self.test_mchirp_song.tracks), ("I", "II")
        )
    ...
    def test_measures(self):
        ...
        self.assertEqual(
            sum(len(t.measures) for t in self.test_mchirp_song.tracks), 104
        )
    ...
    def test_conversion(self):
        ...
        self.assertEqual(
            [t.name for t in chirp_song.tracks], [t.name for t in self.test_song.tracks]
        )
    ...
    def test_triplets(self):
        test_song = ctsMidi.MIDI().to_chirp(
            project_to_absolute_path("test/data/tripletTest.mid")
        )
        ...
        total_triplets = sum(
            1
            for t in test_mchirp.tracks
            for m in t.measures
            for e in m.events
            if isinstance(e, Triplet)
        )
