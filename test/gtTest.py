...
class TestGoatTrackerFunctions(unittest.TestCase):
    ...
    def found_expected_note_content(self, rchirp_song):
        ...
        for i, expected_channel in enumerate(EXPECTED_CHANNELS):
            with self.subTest(i=i):
                actual_channel = {
                    v.jiffy_num: v for k, v in rchirp_song.voices[i].rows.items()
                }
                for expected_jiffy, expected_note in expected_channel:
                    ...
                    actual_note = ctsBase.pitch_to_note_name(
                        rchirp_row.note_num
                    )  # do enharmonic comparison
    ...
    def test_sng_to_rchirp_to_sng_to_rchirp(self):
        ...
        parsed_gt2.export_rchirp_to_parsed_gt(
            rchirp_song, end_with_repeat=False, max_pattern_len=126
        )
        ...
        self.assertTrue(
            self.parsed_gt.get_instruments_bytes() == parsed_gt2.get_instruments_bytes()
        )
        self.assertTrue(
            self.parsed_gt.wave_table.to_bytes() == parsed_gt2.wave_table.to_bytes()
        )
        self.assertTrue(
            self.parsed_gt.pulse_table.to_bytes() == parsed_gt2.pulse_table.to_bytes()
        )
        self.assertTrue(
            self.parsed_gt.filter_table.to_bytes() == parsed_gt2.filter_table.to_bytes()
        )
        self.assertTrue(
            self.parsed_gt.speed_table.to_bytes() == parsed_gt2.speed_table.to_bytes()
        )
    ...
    def test_add_instrument(self):
        ...
        ctsGoatTracker.add_gt_instrument_to_rchirp(
            rchirp_song, "SlepBass", "test/data/"
        )

        self.assertTrue(
            extensions["gt.wave_table"][0] == 2 + 4
            and extensions["gt.pulse_table"][0] == 5  # adds 4
            and extensions["gt.filter_table"][0] == 6  # adds 5
            and extensions["gt.speed_table"][0] == 1  # adds 6
        )  # adds 1
...
if __name__ == "__main__":
    ...
    unittest.main(
        failfast=True
    )  # Lots of asserts (in a loop), so stop after first fail
