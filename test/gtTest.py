...
class TestGoatTrackerFunctions(unittest.TestCase):
    ...
    def test_sng_parsing(self):
        ...
        for i, channel in enumerate(channels):
            for tup in channel:
                # do enharmonic note comparisons
                parsed_notes = channels_time_events[i][tup[0]].note
                asserted_note = note_name_to_pitch(tup[1])                
                self.assertEqual(parsed_notes, asserted_note)
