    ...
    for resource in resources:
        ...
        if os.path.exists(local_file):
            if not os.path.isfile(local_file):
                raise Exception(
                    'Error: not expecting "%s" to not be a file' % (local_file)
                )
            if SKIP_IF_EXISTS:
        ...
        try:
            ...
            print(
                'Unable to download file at "%s", due to exception "%s"'
...
def main():
    ...
    resources.append(
        ResourceFile(an_hvsc_mirror + "/D/Dunbar_Tommy/Archon.sid", "test/sid")
    )

    resources.append(
        ResourceFile(an_hvsc_mirror + "/B/Boles_Howard/Dragonworld.sid", "test/sid")
    )

    resources.append(
        ResourceFile(an_hvsc_mirror + "/F/Fulton_Douglas/Skyfox.sid", "test/sid")
    )

    resources.append(
        ResourceFile(
            an_hvsc_mirror + "/L/Lieblich_Russell/Master_of_the_Lamps_PAL.sid",
            "test/sid",
        )
    )
    ...
    resources.append(
        ResourceFile(an_hvsc_mirror + "/N/Norman_Paul/Super_Huey.sid", "test/sid")
    )

    resources.append(
        ResourceFile(an_hvsc_mirror + "/W/Warhol_Dave/Pool_of_Radiance.sid", "test/sid")
    )

    resources.append(
        ResourceFile(
            "http://youdzone.com/testData/appleii/u4/must",
            "apps/ultima/data/appleii_u4",
        )
    )

    resources.append(
        ResourceFile(
            "http://youdzone.com/testData/appleii/u4/muso",
            "apps/ultima/data/appleii_u4",
        )
    )
    ...
    resources.append(
        ResourceFile(
            "http://youdzone.com/testData/appleii/u4/musd",
            "apps/ultima/data/appleii_u4",
        )
    )
    ...
    resources.append(
        ResourceFile(
            "http://youdzone.com/testData/appleii/u4/musc",
            "apps/ultima/data/appleii_u4",
        )
    )
    ...
    resources.append(
        ResourceFile(
            "http://youdzone.com/testData/appleii/u4/musb",
            "apps/ultima/data/appleii_u4",
        )
    )
